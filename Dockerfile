...
MAINTAINER Shivam Mishra <shivmishr78@gmail.com>

RUN apt-get update
RUN apt-get install openjdk-8-jdk -y
RUN apt-get install -y wget
RUN apt-get install -y openssh-server
RUN wget https://archive.apache.org/dist/hadoop/core/hadoop-3.1.3/hadoop-3.1.3.tar.gz -P ~/Downloads
RUN tar zxvf ~/Downloads/hadoop-3.1.3.tar.gz  -C /usr/local
